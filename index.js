...
// 🎉 Giveaway storage (temporary memory, can move to DB later)
let giveaways = [];
...
client.on("messageCreate", async (message) => {
    ...
    if (cmd === "warn") {
        ...
        await db.query(
          "INSERT INTO warnings(user_id, reason, guild_id) VALUES($1, $2, $3)",
          [user.id, reason, message.guild.id]
        );
    ...
    if (cmd === "warnings") {
        ...
        const result = await db.query(
          "SELECT reason, created_at FROM warnings WHERE user_id = $1 AND guild_id = $2",
        ...
        const warningList = result.rows.map((row, i) => `${i+1}. ${row.reason} (${row.created_at.toLocaleString()})`).join("\n");

        return message.channel.send(`⚠️ ${user.tag} has ${result.rows.length} warnings:\n${warningList}`);
    }
